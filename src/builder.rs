...
use crate::{
    ...
    nodes::{AttachedTrivia, SyntaxKind, Tree, TreeInner},
    api::{Language, SyntaxTree},
};
...
impl Drop for TreeBuilder {
    fn drop(&mut self) {
        if !std::thread::panicking() {
            ...
            // Clear the instance.
            *self = Self::new(Box::default(), SyntaxKind(u16::MAX));
        }
    }
}
...
impl TreeBuilder {
    pub fn new(full_text: Box<str>, root_kind: SyntaxKind) -> Self {
        let mut nodes = Vec::with_capacity(DEFAULT_TREE_SIZE);
        nodes.push(Node {
            parent: None,
            children: ManuallyDrop::new(Vec::with_capacity(DEFAULT_CHILDREN_LEN)),
            children_ptr: 0,
            children_len: 0,
            kind: root_kind,
    ...
    pub fn start_node(&mut self, kind: SyntaxKind) {
        ...
        match self.last_opened() {
            Opened::Node(direct_parent) => {
                self.nodes[direct_parent].children.push(ChildKind::Node(new_node))
            }
            Opened::List(direct_parent) => self.lists[direct_parent].children.push(new_node),
        }
        self.opened.push(Opened::Node(new_node));
    }

    fn last_parent(&mut self) -> usize {
        self.opened
            .iter()
            .rev()
            ...
            .expect("there are always opened nodes")
    }

    pub fn finish_node(&mut self) {
        if self.opened.len() == 1 {
            // Check here and not after the pop so that we won't create an invalid builder which can be used later by recovering
            // from the panic.
            panic!("can't finish the root node; instead call `finish()` on the builder itself");
        }
        self.finish_node_impl();
    }

    fn finish_node_impl(&mut self) {
        ...
        self.node_children.extend(children.drain(..));
    ...
    pub fn finish_list(&mut self) {
        ...
        list.children_ptr = self.node_children.len();
        list.children_len = children.len().try_into().unwrap();
        self.list_children.extend(children.drain(..));
    ...
    pub fn token(
        ...
        self.tokens.extend(leading_trivia.map(|(kind, text_len)| Token {
            kind,
            attached_trivia: AttachedTrivia::new(false, false, 0),
        ...
        self.tokens.extend(trailing_trivia.map(|(kind, text_len)| Token {
            kind,
    ...
    fn update_first_last_tokens(&mut self, first_token: usize, last_token: usize) {
        ...
        for ancestor in &self.opened {
            ...
            match &mut node.first_last_token {
                ...
                // There was already a token. It is the first token, but we're after it so (maybe) we're last.
                Some((_f, l)) => *l = last_token,
            }
        }
    }

    pub fn finish<L: Language>(self) -> SyntaxTree<L> {
        SyntaxTree { tree: self.finish_impl(), _marker: PhantomData }
    }

    #[expect(unsafe_code)]
    fn finish_impl(mut self) -> Tree {
        assert_eq!(self.opened.len(), 1);
...
///
/// Note: you must use the returned len, not a len you saved from before (otherwise a malicious iterator
/// could return different lengths).
#[inline]
