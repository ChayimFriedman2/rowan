...
use crate::nodes::{SyntaxKind, node::Node, tree::TreeInner};
...
pub(crate) struct AttachedTrivia {
    // Encoding: the LSB is 1 when there is leading trivia, and 0 if not. The second LSB is the same for trailing trivia.
    // Other bits encode:
    //
    //  - If this is a non-trivia token, the number of leading trivia tokens.
    //  - If this is the first trailing trivia token of some token (whose LSB is 1), the number
    //    of trailing trivia tokens of that number.
    raw: u16,
}

impl AttachedTrivia {
    const MAX_TRIVIAL_LEN: usize = ((1 << u16::BITS) - 1);
    ...
    pub(crate) fn new(
        ...
        assert!(trivia_len <= Self::MAX_TRIVIAL_LEN);
        AttachedTrivia {
            raw: ((trivia_len << 2)
...
impl<'a> TokenRef<'a> {
    ...
    // Doesn't return a `TokenRef`, because it's invariant is that it's not fake.
    #[inline]
    ...
    #[inline]
    pub(crate) fn prev_token(self, tree: &TreeInner) -> Option<TokenRef<'a>> {
        let prev_token = unsafe { self.ptr.sub(1) };
        if tree.tokens.as_ptr() == prev_token {
            None
        } else {
            Some(TokenRef { ptr: prev_token, _marker: PhantomData })
        }
    }

    #[inline]
    pub(crate) fn next_token(self, tree: &TreeInner) -> Option<TokenRef<'a>> {
