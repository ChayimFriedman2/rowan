...
use crate::{
    ...
    utility_types::{NodeOrToken, TokenAtOffset},
};
...
impl NodeOrListOrToken {
    ...
    #[inline]
    pub(crate) fn new_token(ptr: *const Token) -> NodeOrListOrToken {
        NodeOrListOrToken(ptr.map_addr(|addr| addr | Self::TOKEN_TAG).cast())
    }

    #[inline]
    pub(crate) fn new_node(ptr: *const Node) -> NodeOrListOrToken {
        NodeOrListOrToken(ptr.map_addr(|addr| addr | Self::NODE_TAG).cast())
    }

    #[inline]
    pub(crate) fn new_list(ptr: *const List) -> NodeOrListOrToken {
        NodeOrListOrToken(ptr.map_addr(|addr| addr | Self::LIST_TAG).cast())
    }
